  ...
  const loadDashboardData = async () => {
    try {
      ...
      if (user?.role === "ADMIN") {
        ...
        mappedData = {
          pendingLoans: data?.totalPendingReview || 0,
          totalLoans: (data?.pendingVehicleLoans || 0) + (data?.pendingStudentLoans || 0),
    ...
    <div className="container">
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          ...
          <a href="/loan-application" className="btn btn-success">
            Apply for Loan
          </a>
          <a href="/loans" className="btn btn-info">
            My Loans
          </a>
          {(user?.role === "ADMIN" || user?.role === "LOAN_OFFICER") && (
            <a href="/loan-management" className="btn btn-primary">
              Manage Loans
            </a>
          )}
        </div>
